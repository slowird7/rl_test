...
class AhonokoTank(Env):
    """
    Ahonoko Tank involves crossing a Ahonoko Tank from start to goal without falling into any holes
    by walking over the Ahonoko Tank.
    The player may not always move in the intended direction due to the slippery nature of the Ahonoko Tank.
    ## Description
    The game starts with the player at location [0,0] of the Ahonoko Tank grid world with the
    ...
    * v0: Initial version release
    ...
    def __init__(
        ...
        def to_s(row, col, dir):
            return row * ncol * dir + col * dir + dir

        def inc(row, col, dir, a):
            if a == TURNRIGHT:
                dir = (dir + 1) % 4
            elif a == WEST:
                dir = (dir - 1) % 4
        ...
        def update_probability_matrix(row, col, dir, action):
            newrow, newcol, newdir = inc(row, col, dir, action)
    ...
    def _render_gui(self, mode):
        ...
        if self.window_surface is None:
            ...
            if mode == "human":
                pygame.display.init()
                pygame.display.set_caption("Ahonoko Tank")
        ...
        if self.wall_img is None:
            file_name = path.join(path.dirname(__file__), "img/wall.png")
        ...
        if self.field_img is None:
            file_name = path.join(path.dirname(__file__), "img/field.png")
        ...
        if self.tank_images is None:
            tanks = [
                path.join(path.dirname(__file__), "img/car_north.png"),
                path.join(path.dirname(__file__), "img/car_east.png"),
                path.join(path.dirname(__file__), "img/car_south.png"),
                path.join(path.dirname(__file__), "img/car_west.png"),
            ]
            self.tank_images = [
        ...
        for y in range(self.nrow):
            for x in range(self.ncol):
                ...
                self.window_surface.blit(self.field_img, pos)
                if desc[y][x] == b"H":
        ...
        elf_img = self.tank_images[last_action]
